...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof emailjs !== 'undefined') {
    try {
      emailjs.init('LFvuVRN2EPNl435py'); // <-- replace
    } catch (e) {
      // ignore init errors
    }
  }
  ...
  if (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (typeof emailjs === 'undefined') {
        ...
        window.location.href = `mailto:prashanthnarava@gmail.com?subject=Contact from ${encodeURIComponent(name)}&body=${encodeURIComponent(msg + '\n\nFrom: ' + mail)}`;
        return;
      }
